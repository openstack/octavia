...
class TestServerTestCase(base.TestCase):
    ...
    def _test_plug_network(self, distro, mock_isfile, mock_int_exists,
        ...
        netns_handle.get_links.return_value = [
            {'attrs': [['IFLA_IFNAME', 'eth{idx}'.format(idx=idx)]]}
        ...
        with mock.patch('os.open') as mock_open, mock.patch.object(
            ...
            if distro == consts.UBUNTU:
                handle.write.assert_any_call(
                    ...
                    '-o eth{int} -j MASQUERADE\n\n'
                    'post-up /sbin/iptables -t raw -A PREROUTING -p tcp '
                    '-i eth{int} -j NOTRACK\n'
                    'post-up /sbin/iptables -t raw -A OUTPUT -p tcp '
                    '-o eth{int} -j NOTRACK\n'
                    'post-down /sbin/iptables -t raw -D PREROUTING -p tcp '
                    '-i eth{int} -j NOTRACK\n'
                    'post-down /sbin/iptables -t raw -D OUTPUT -p tcp '
                    '-o eth{int} -j NOTRACK\n'.format(int=test_int_num))
            elif distro == consts.CENTOS:
                handle.write.assert_any_call(
                    '\n\n# Generated by Octavia agent\n'
        ...
        with mock.patch('os.open') as mock_open, mock.patch.object(
            ...
            if distro == consts.UBUNTU:
                handle.write.assert_any_call(
                    ...
                    '-o eth{int} -j MASQUERADE\n\n'
                    'post-up /sbin/ip6tables -t raw -A PREROUTING -p tcp '
                    '-i eth{int} -j NOTRACK\n'
                    'post-up /sbin/ip6tables -t raw -A OUTPUT -p tcp '
                    '-o eth{int} -j NOTRACK\n'
                    'post-down /sbin/ip6tables -t raw -D PREROUTING -p tcp '
                    '-i eth{int} -j NOTRACK\n'
                    'post-down /sbin/ip6tables -t raw -D OUTPUT -p tcp '
                    '-o eth{int} -j NOTRACK\n'.format(int=test_int_num))
            elif distro == consts.CENTOS:
                handle.write.assert_any_call(
                    '\n\n# Generated by Octavia agent\n'
    ...
    def _test_plug_network_host_routes(self, distro, mock_check_output,
        ...
        mode = stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH
        if distro == consts.UBUNTU:
            file_name = '/etc/netns/{0}/network/interfaces.d/eth3.cfg'.format(
                consts.AMPHORA_NAMESPACE)
        elif distro == consts.CENTOS:
            file_name = ('/etc/netns/{0}/sysconfig/network-scripts/'
                         'ifcfg-eth3'.format(consts.AMPHORA_NAMESPACE))
        m = self.useFixture(test_utils.OpenFixture(file_name)).mock_open
        with mock.patch('os.open') as mock_open, mock.patch.object(
                os, 'fdopen', m) as mock_fdopen:
            mock_open.return_value = 123
